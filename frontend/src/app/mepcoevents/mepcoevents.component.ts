...
@Component({
  selector: 'app-mepcoevents',
  templateUrl: './mepcoevents.component.html',
  styleUrls: ['./mepcoevents.component.css']
})
export class MepcoeventsComponent implements OnInit {
  ...
  listOfEvents=[{title:'Mepcoevent', name:'',detail:'',location:'',map:'',date:'',img:'',no:''}];
  formEvent={title:'Mepcoevent', name:'',detail:'',location:'',map:'',date:'',img:'',no:''};
  updateEvent={title:'Mepcoevent', name:'',detail:'',location:'',map:'',date:'',img:'',no:''};

  insertString:string='';
  alert:string='';
  selectedNo:number=0;
  change={title:'Mepcoevent',name:'',value:''};
  ...
  formClicked() {
    ...
    this.formEvent={title:'Mepcoevent', name:'',detail:'',location:'',map:'',date:'',img:'',no:''};
  }
  ...
  formSubmitClicked() {
    ...
    this.formEvent.img="../../assets/img/mepcoevents/"+this.formEvent.img.replace("C:\\fakepath\\","");
  ...
  imgClicked() {
    this.saveFlag=true;
    this.updateEvent.img="../../assets/img/mepcoevents/"+this.updateEvent.img.replace("C:\\fakepath\\","");
  ...
  updateSubmitClicked() {
    if(this.saveFlag) {
      this.mydb.updateEducationService(this.formEvent.name,this.change).subscribe((data: { [x: string]: any; })=> {
        if(data['message']) {
